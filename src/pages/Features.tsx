          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {features.map((feature, index) => (
              <motion.div 
                ...
                className={`rounded-xl p-6 group transition-all duration-500 relative overflow-hidden border-2 ${goldStyle.border} ${goldStyle.bg} ${goldStyle.shadow} ${goldStyle.shadowHover}`}
              >
                ...
                <div className={`absolute top-0 left-0 w-6 h-6 border-t-2 border-l-2 ${goldStyle.accent} opacity-50 rounded-tl-lg`} />
                <div className={`absolute top-0 right-0 w-6 h-6 border-t-2 border-r-2 ${goldStyle.accent} opacity-50 rounded-tr-lg`} />
                <div className={`absolute bottom-0 left-0 w-6 h-6 border-b-2 border-l-2 ${goldStyle.accent} opacity-50 rounded-bl-lg`} />
                <div className={`absolute bottom-0 right-0 w-6 h-6 border-b-2 border-r-2 ${goldStyle.accent} opacity-50 rounded-br-lg`} />
                ...
                <div className="absolute inset-0 overflow-hidden pointer-events-none">
                  {[...Array(4)].map((_, i) => (
                    <motion.div
                      key={i}
                      className={`absolute w-1 h-1 rounded-full ${goldStyle.particle}`}
                      style={{
                        left: `${15 + i * 20}%`,
                        top: `${25 + (i % 2) * 35}%`,
                      }}
                ...
                <div className={`relative z-10 w-12 h-12 rounded-lg ${goldStyle.iconBg} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}>
                  <feature.icon className={`w-6 h-6 ${goldStyle.iconColor}`} />
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {configFeatures.map((item, index) => (
                <motion.div 
                  ...
                  transition={{ delay: index * 0.05 }}
                  whileHover={{ scale: 1.02, y: -3 }}
                  className={`rounded-xl p-5 group transition-all duration-500 relative overflow-hidden border-2 ${goldStyle.border} ${goldStyle.bg} ${goldStyle.shadow} ${goldStyle.shadowHover}`}
                >
                  {/* Shimmer effect overlay */}
                  <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
                  ...
                  {/* Animated corner accents */}
                  <div className={`absolute top-0 left-0 w-5 h-5 border-t-2 border-l-2 ${goldStyle.accent} opacity-50 rounded-tl-lg`} />
                  <div className={`absolute top-0 right-0 w-5 h-5 border-t-2 border-r-2 ${goldStyle.accent} opacity-50 rounded-tr-lg`} />
                  <div className={`absolute bottom-0 left-0 w-5 h-5 border-b-2 border-l-2 ${goldStyle.accent} opacity-50 rounded-bl-lg`} />
                  <div className={`absolute bottom-0 right-0 w-5 h-5 border-b-2 border-r-2 ${goldStyle.accent} opacity-50 rounded-br-lg`} />
